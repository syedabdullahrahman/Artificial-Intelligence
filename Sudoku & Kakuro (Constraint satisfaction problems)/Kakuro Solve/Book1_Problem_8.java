...
// 1. Create a Model
        Model model = new Model("Book 1 Problem No. 8");
// 2. Create variables
...
// 3. Post constraints
 ...
 /* a total of 6 vertical sum clues for this problem */
        ...
        IntVar[] c1 = model.intVarArray("c1", 5, 1, 9);
 ...
 /* for example r0 [0] + r0 [1] = bd [1, 3] + bd [1, 4] = 15*/
        ...
        model.sum(r0, "=", 10).post();
        model.sum(r1, "=", 23).post();
        model.sum(r2, "=", 12).post();
        model.sum(r3, "=", 10).post();
        model.sum(r4, "=", 26).post();
        ...
        for (i = 0; i < 2; i++) {
            model.arithm(r2[i], ">", 2).post();
        }
        for (i = 0; i < 4; i++) {
            model.arithm(r4[i], ">", 1).post();
        }
        for (i = 0; i < 3; i++) {
            model.arithm(r5[i], "<", 9).post();
        }
 ...
 /* for example c0 [0] + c0 [1] = bd [1, 3] + bd [2, 3] = 17  */
        ...
        model.sum(c0, "=", 5).post();
        model.sum(c1, "=", 23).post();
        model.sum(c2, "=", 3).post();
        model.sum(c3, "=", 17).post();
        model.sum(c4, "=", 27).post();
        model.sum(c5, "=", 17).post();


        /* for second formulation */
        for (i = 0; i < 2; i++) {
            model.arithm(c0[i], "<", 5).post();
        }
        for (i = 0; i < 2; i++) {
            model.arithm(c2[i], "<", 3).post();
        }
        for (i = 0; i < 2; i++) {
            model.arithm(c3[i], ">", 7).post();
        }
        for (i = 0; i < 2; i++) {
            model.arithm(c5[i], ">", 7).post();
        }
